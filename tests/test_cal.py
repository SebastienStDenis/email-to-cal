...
def test_a_time_in_the_operators_own_zone_is_not_labelled(preferences: Prefs) -> None:
    event = ExtractedEvent(
        kind="appointment",
        title="Dentist",
        all_day=False,
...
def test_a_venue_named_after_a_city_does_not_move_the_event(preferences: Prefs) -> None:
    ...
    built = build_ical(event, preferences, message_id="<a@b>")
    assert "DTSTART;TZID=Europe/Zurich:20260914T123000" in ical_text(event, preferences)
    assert built.describe() == "Lunch - Mon 14 Sep 12:30"


def test_the_city_field_still_resolves_the_zone(preferences: Prefs) -> None:
    event = ExtractedEvent(
        kind="concert",
        title="Radiohead",
        all_day=False,
    ...
    assert "DTSTART;TZID=Europe/London:20260914T200000" in ical_text(event, preferences)
